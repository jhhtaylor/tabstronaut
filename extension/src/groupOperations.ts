...
export async function selectTabGroup(
  ...
  const newGroupLabel =
    fileUris.length > 0
      ? `New Tab Group from selected ${fileUris.length > 1 ? 'files' : 'file'}...`
      : 'New Tab Group from current tab...';

  const newGroupTooltip =
    fileUris.length > 0
      ...
      if (item.id && item.id !== 'new') {
        ...
        if (fileUris.length > 0) {
          for (const uri of fileUris) {
            await treeDataProvider.addToGroup(group.id, uri.fsPath);
          }
          showConfirmation(
            `Added ${fileUris.length} file(s) to Tab Group '${group.label}'.`
          );
          quickPick.hide();
          return;
        }
...
export async function addFilesToGroupCommand(
  ...
  const selectedGroup = await selectTabGroup(treeDataProvider, fileUris);
  if (!selectedGroup) {
    return;
  }
